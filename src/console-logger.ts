...
function getLogStyle(options: FinalOptions | null): string {
	return options?.logStyle ?? "color: black";
}

function getWarnStyle(options: FinalOptions | null): string {
	return options?.warnStyle ?? "color: orange";
}
...
export function patchConsole(patchOptions: PatchOptions = {}): void {
	...
	const outputLog = (
		...
		const textStyle = getLogStyle(options);
		if (options.prefix) {
			originalConsole?.log(
				`%c${options.prefix}%c${str}`,
				getPrefixStyle(options),
				textStyle,
				...args,
			);
		} else {
	...
	const outputWarn = (
		...
		const textStyle = getWarnStyle(options);
		if (options.prefix) {
			...
			originalConsole?.warn(`%c${str}`, textStyle, ...args);
		}
	};
	...
	const outputError = (
		...
		const textStyle = getErrorStyle(options);
		if (options.prefix) {
			...
			originalConsole?.error(`%c${str}`, textStyle, ...args);
		}
	};
